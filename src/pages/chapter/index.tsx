...
import { StarOutline, UpOutline, DownOutline } from 'antd-mobile-icons'
...
const Page = ({ elist, cLists, dispatch }) => {
  const [SearchParams] = useSearchParams();
  const navigate = useNavigate();
  const { data: cdata, loading } = useRequest(() => getChapter(Number(SearchParams.get('id')!)), { cacheKey: 'chapter' });
  ...
  const GoContent = (need: boolean, id: string, title: string) => {
    if (need) {
      Toast.show({
    ...
    <div>
      ...
      <Tabs>
        <Tabs.Tab title='章节列表' key='lists'>
          {
            cLists && cLists.map((item: any, index2: number) => {
              return <div key={index2} className={styles.listitem} onClick={() => GoContent(item.need_vip, item.id, item.title)}>
                <div>
                  <img className={styles.listimg} src={item.cover_image_url} />
                </div>
                <div>
                  <h3 className={styles.listitle}>{item.title}</h3>
                  <p className={styles.data}><span className={styles.span}>{item.need_vip ? item.label_info.text : ""}</span>{item.created_at}</p>
                </div>
              </div>
            })
          }
      ...
      {/* {
        elist && elist.map((item: any,index:number) => {
          return <div key={index}>
            {item}
            <Card>
              <div className={styles.content}>内容：{item.values.text}</div>
              {
                item && item.values.url.map((item2: any, index1: number) => {
                  return <span className={styles.image} onClick={() => { setVisible1(true) }} key={index1}>
                    <img src={item2.url} className={styles.imagess}></img>
                  </span>
                })
...
export default connect(({ evaluate, chaplist }) => ({
